...
public:
	...
	{
		graph.untag_all();
		std::stack<const kmint::graph::basic_node<kmint::map::map_node_info>*> shortestPath = std::stack<const kmint::
			graph::basic_node<kmint::map::map_node_info>*>{};
		...
		std::priority_queue<std::pair<int, const kmint::graph::basic_node<kmint::map::map_node_info>*>,
		                    ...
		                    std::greater<std::pair<int, const kmint::graph::basic_node<kmint::map::map_node_info>*>>>
			priorityQueue;
		...
		for (std::size_t i = 0; i < graph.num_nodes(); ++i)
		{
			...
			distances.insert(
				std::pair<kmint::graph::basic_node<kmint::map::map_node_info>*, distance>(&graph[i], dist));
		}
		...
		while (!priorityQueue.empty())
		{
			...
			// get all adjacent vertices of a node
			for (auto& vertex : *node)
			{
				...
				{
					//  If there is a shorter path to oppositenode through node. 
					if (distances.at(oppositeNode).shortestDistance() > distances.at(node).shortestDistance() + vertex.
						weight() + distanceBetween(oppositeNode->location(), end.location()))
					{
						// Updating distance of v 
						distances.at(oppositeNode).setShortestDistance(
							distances.at(node).shortestDistance() + vertex.weight() + distanceBetween(
								oppositeNode->location(), end.location()));
