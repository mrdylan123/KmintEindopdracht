	...
	namespace pigisland
	{
		shark::shark(kmint::map::map_graph& g)
			: play::map_bound_actor{g, find_shark_resting_place(g)},
			  drawable_{*this, shark_image()}, map_{&g}, resting_place_(&node()), totalSteps_{0}, isResting_{false},
			  isScared_{false}, isHungry_{true}
		{
			//shortest_path_ = aStar::run(g, *resting_place_, g[57]);

			currentState_.push(std::make_unique<sharkWanderState>(this));
			std::cout << "Knabbel state: " << typeid(*currentState_.top()).name() << std::endl;
		}
